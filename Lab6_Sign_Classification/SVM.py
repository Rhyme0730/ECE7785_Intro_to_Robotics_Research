...
def extract_hog_features(image):
    ...
    winSize = (30, 30)  # Assuming your images are resized to 30x30
    blockSize = (10, 10)
    blockStride = (5, 5)
    cellSize = (10, 10)
    nbins = 9
    ...
    return hog_features.flatten()  # Flatten the features array

def train_SVM(train_lines, imageDirectory = './2023Fimgs/', image_type = '.jpg'):
    ...
    for i in range(len(train_lines)):
        ...
        cropped = crop(image) 
        hog_features = extract_hog_features(cropped)  # Extract HOG features
        train.append(hog_features)

    train_data = np.array(train)
    train_data = train_data.flatten().reshape(len(train_lines), -1)
    ...
    svm.setTermCriteria((cv2.TERM_CRITERIA_MAX_ITER, 400, 1e-6))
    ...
    for i in range(len(train_lines)):
        ...
        test_img = extract_hog_features(test_img)
        ret, prediction = svm.predict(test_img)
        ret = prediction[0][0]
...
def test_SVM(svm, test_lines, imageDirectory = './2023Fimgs/', image_type = '.jpg'):
    correct = 0
    confusion_matrix = np.zeros((6,6))
...
def train_test_data():
    file_path = './img_directory/2023Fimgs/labels.txt'
    ...
    train_file_path = './img_directory/2023Fimgs/train.txt'
    test_file_path = './img_directory/2023Fimgs/test.txt'
...
def main():
    ...
    # svm = train_SVM(train, directory, image_type='.png')
    # test_SVM(svm, test, directory, image_type='.png')

    svm = train_SVM(train, directory)
    test_SVM(svm, test, directory)
