...
def find_tangents(point, circle_center, radius):
    """
    Calculate distance between point and circle center
    Check if point is outside, on, or inside the circle
    Calculate angles of the tangential lines
    Find coordinates of the tangent points
    """

    d = math.sqrt((point[0] - circle_center[0])**2 + (point[1] - circle_center[1])**2)

    if math.isclose(d, radius):
        return [(point[0], point[1])]
    elif d < radius:
        return []

    theta = math.atan2(point[1] - circle_center[1], point[0] - circle_center[0])
    alpha = math.acos(radius / d)

    tangent_point1 = (
        circle_center[0] + radius * math.cos(theta + alpha),
        circle_center[1] + radius * math.sin(theta + alpha)
    )
...
class GoalGoer(Node):
    def __init__(self):
        ...
        # Distance and Angular Position Publisher
        self.move_publisher = self.create_publisher(Twist, '/cmd_vel', 1)

        # control timing/callback
        self.dt = 0.05
        self.timer = self.create_timer(self.dt, self.timer_callback)

        # state in global frame: (x, y, theta)
        self.g_WR = np.zeros(3)
        # goals in world frame:
        self.goals = np.array([[1.75, 0], [1.75, 1.6], [0, 1.57]])
    ...
    def timer_callback(self):
        if self.subgoal is None:
            ...
            # go fowrad until the path is free 
            msg = Twist()
            msg.linear.x = 0.1
            self.move_publisher.publish(msg)


        elif self.edge or np.linalg.norm(self.subgoal)<self.width*1.5:
            print(2)
            # travel along the edge
            self.edge_travel()
            self.edge = True

            if np.linalg.norm(self.subgoal) > 1.5*self.width:
                self.rotated_90 = True
         
        else:
            # move to subgoal first to move around object
