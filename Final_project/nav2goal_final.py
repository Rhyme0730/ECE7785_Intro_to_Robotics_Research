...
class nav2goal_node(Node):
    def __init__(self):
        ...
        # need to change this!!!!!!!!
        self.feedback.current_pose.pose.position.x = 0.32
        self.feedback.current_pose.pose.position.y = -0.72
    ...
    def check_ang(self, ori):
        ...
        # print(f'curr_ang = {x, y, z, w}, goal_ang = {self.goal.pose.orientation}')
        # if np.abs(x-ori[0]) < 0.1 and np.abs(y-ori[1]) < 0.1 and np.abs(z-ori[2]) < 0.1 and np.abs(w-ori[3]) < 0.1:
        #     return True
        print(f'curr_ang = {curr_ori}, goal_ang={goal_ori}')
        if np.abs(curr_ori-goal_ori) < 0.2:
            return True
        return False
